...
func (s *AttendanceService) GetAttendanceDailyByDate(date string) ([]dto.AttendanceDailyDate, error) {
	...
	for _, emp := range employee {
		if !presentSet[emp.ID] {
			...
			absentList = append(absentList, absent)
		}
	}

	var result []dto.AttendanceDailyDate
	result = append(result, dto.AttendanceDailyDate{
		Date:         date,
		PresentDaily: presentList,
		AbsentDaily:  absentList,
		PresentToday: len(presentList),
		AbsentToday:  len(absentList),
	})
